...
function loadStatsPage(){
	...
	var win_count = 0;
	var lose_count = 0;
	for(var row_counter = 2 /*skip the first two rows*/; row_counter < table.rows.length; row_counter++){
			var opposing_team_name = table.rows[row_counter].cells[1].innerHTML;
			var score_one = parseInt(table.rows[row_counter].cells[2].innerHTML);
			var score_two = parseInt(table.rows[row_counter].cells[3].innerHTML);
			if(score_one > score_two){
				table.rows[row_counter].cells[4].innerHTML = "CU Boulder";
				win_count += 1;
			}
			else if(score_one < score_two){
				table.rows[row_counter].cells[4].innerHTML = opposing_team_name;
				lose_count += 1;
			}
	}
...
function loadPlayersPage(){
	...
	for(var playerID = 0; playerID < players.length; playerID++){
			displayedText += '<a class="dropdown-item" href="#" onclick = "switchPlayers(' + playerID + ')">' + players[playerID].name + '</a>';
	}
	return (document.getElementById("player_selector").innerHTML = displayedText);
}
